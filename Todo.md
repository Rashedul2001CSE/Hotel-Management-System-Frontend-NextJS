1. Make the mobile menu button text color more black as  it is not visible in the light theme with dark section of landing page 

2.there are some console log  in the code remove them  

3. there will be this error while server is not up "[browser] Failed to get current user: TypeError: NetworkError when attempting to fetch resource. (src/providers/AuthContext.tsx:70:9)" make it a user friendly toast message 

4.hero component is loading step by step not smoothly fix this 

5. theme is not applying back to it's previous theme while going froma  404 page to home it is going back to light always. fix this 
6. as 5 theme is not working while going from switching from different layout 